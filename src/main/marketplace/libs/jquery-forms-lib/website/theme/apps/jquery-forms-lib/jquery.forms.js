            ...
            return $(this).each(function () {
                ...
                form.off('submit').on('submit', function (e) {
                    ...
                    form.find('.contenteditor').each(function () {
                        var editor = $(this);
                        editor.val(editor.contentEditor('getContent'));
                    });
...
function doPostForm(form, config, event) {
    ...
    try {
            ...
            ajaxOpts.xhr = function () {
                ...
                xhr.upload.addEventListener("progress", function (evt) {
                    if (evt.lengthComputable) {
                        ...
                        config.onProgress.call(this, percentComplete, form);
                    }
                }, false);
                return xhr;
            };
        ...
        if (typeof config.onError === 'function') {
            ...
            alert(config.cantSubmitFormErrorMessage);
        }
    }
}
...
function checkNumeric(target, form, config) {
    ...
    if (value) {
        if (!isNumber(value)) {
            showErrorField(target);
            showErrorMessage(form, config, config.numberErrorMessage);
...
function checkChars(form, inpClass, reg) {
    flog('=======================================================================');
    flog('No need to use "checkChars" any more. For check simple characters or really simple characters, please use "checkSimpleValue" or "checkReallySimpleValue". "checkChars" will be removed in version 1.2.0');
    flog('=======================================================================');
    flog("checkChars");
